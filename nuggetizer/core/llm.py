...
class LLMHandler:
    """Synchronous OpenAI / Azure OpenAI chat-completions client with disk caching."""
    ...
    def __init__(
        ...
        cache_path: str = "/llm_cache/nuggets.pkl",
        ...
        self.client = self._initialize_client(api_base, api_version)
        self.consider_cache = consider_cache
        self.cache = LMCache(cache_path)

    def _initialize_client(self, api_base, api_version):
        if self.api_type == "azure" and all([api_base, api_version]):
            return AzureOpenAI(

...
from ..prompts import (
    ...
    create_nugget_prompt_ungrounded, get_nugget_prompt_content_ungrounded,
    create_nugget_prompt_grounded, get_nugget_prompt_content_grounded,
    create_score_prompt, create_assign_prompt, get_assign_prompt_content
)
from ..core.lm_cache import LMCache
...
class Nuggetizer(BaseNuggetizer):
    def __init__(
        ...
        log_level: int = 0,
        cache_path: str = "/llm_cache/nuggetizer",
        consider_cache: bool = True,
        **llm_kwargs
    ):
        ...
        # print("CONSIDER CACHE IS " + str(consider_cache))
        self.creator_llm = LLMHandler(creator_model, api_keys, cache_path=cache_path+"/creator.pkl", consider_cache=consider_cache, **llm_kwargs)
        self.scorer_llm = LLMHandler(scorer_model, api_keys, cache_path=cache_path+"/scorer.pkl", consider_cache=consider_cache, **llm_kwargs)
        self.assigner_llm = LLMHandler(assigner_model, api_keys, cache_path=cache_path+"/assigner.pkl", consider_cache=consider_cache, **llm_kwargs)
    ...
    def _create_nugget_prompt(self, request: Request, start: int, end: int, nuggets: List[str], grounding=None) -> List[Dict[str, str]]:
        if grounding == "grounded":
            return create_nugget_prompt_grounded(request, start, end, nuggets)
        if grounding == "ungrounded":
            return create_nugget_prompt_ungrounded(request, start, end, nuggets)
        return create_nugget_prompt(request, start, end, nuggets)

    def _get_nugget_prompt_content(self, request: Request, start: int, end: int, nuggets: List[str], grounding=None) -> str:
        if grounding == "grounded":
            return get_nugget_prompt_content_grounded(request, start, end, nuggets, self.creator_max_nuggets)
        if grounding == "ungrounded":
            return get_nugget_prompt_content_ungrounded(request, start, end, nuggets, self.creator_max_nuggets)
    ...
    def create(self, request: Request, grounding=None) -> List[ScoredNugget]:
        if self.log_level >= 1:
        ...
        if grounding == "ungrounded":
            if self.log_level >= 1:
                self.logger.info(f"Creating ungrounded nuggets.")
            
            prompt = self._create_nugget_prompt(request, start, 0, current_nuggets, grounding)
            if self.log_level >= 2:
            ...
            while trial_count > 0:
                try:
                    ...
                    if trial_count < 500:
                        response, _ = self.creator_llm.run(prompt, temperature=temperature, second_try=True)
                    else:
                ...
                try:
                    ...
                    if self.log_level >= 1:
                        self.logger.info(f"Successfully created nuggets, nugget count: {len(current_nuggets)}")
                    break
                except Exception as e:
            ...
            while start < len(request.documents):
                ...
                prompt = self._create_nugget_prompt(request, start, end, current_nuggets, grounding)
                if self.log_level >= 2:
                    self.logger.info(f"Generated prompt:\n{prompt}")
                ...
                while trial_count > 0:
                    try:
                        ...
                        # with open(r"C:\Users\t-mwanner\OneDrive - Microsoft\Desktop\nuggets\data\prompt.txt", 'a') as file:
                        #     file.write("PROMPT: " + str(prompt) + "\n")
        ...
        self.creator_llm.cache.save_cache()
        self.scorer_llm.cache.save_cache()
    ...
    def assign(self, query: str, context: str, nuggets: List[ScoredNugget]) -> List[AssignedScoredNugget]:
        ...
        self.assigner_llm.cache.save_cache()

...
def create_combined_precision_recall_chart(df, 
    ...
    """
    Print Factscore + Vital Precision (precision) and 
    Nuggets Recall + Vital Recall (recall) as a formatted table.
    """
    ...
    # Add precision data
    metrics_info = [
        ('PRECISION', dataset1_label, 'Factscore', dataset1_prec_normal[0], dataset1_prec_missing[0], dataset1_prec_wrong[0]),
        ('PRECISION', dataset1_label, 'Vital Precision', dataset1_prec_normal[1], dataset1_prec_missing[1], dataset1_prec_wrong[1]),
        ('PRECISION', dataset2_label, 'Factscore', dataset2_prec_normal[0], dataset2_prec_missing[0], dataset2_prec_wrong[0]),
        ('PRECISION', dataset2_label, 'Vital Precision', dataset2_prec_normal[1], dataset2_prec_missing[1], dataset2_prec_wrong[1]),
        ('RECALL', dataset1_label, 'Nuggets Recall', dataset1_rec_normal[0], dataset1_rec_missing[0], dataset1_rec_wrong[0]),
        ('RECALL', dataset1_label, 'Vital Recall', dataset1_rec_normal[1], dataset1_rec_missing[1], dataset1_rec_wrong[1]),
        ('RECALL', dataset2_label, 'Nuggets Recall', dataset2_rec_normal[0], dataset2_rec_missing[0], dataset2_rec_wrong[0]),
        ('RECALL', dataset2_label, 'Vital Recall', dataset2_rec_normal[1], dataset2_rec_missing[1], dataset2_rec_wrong[1]),
    ]
    
    for metric_type, dataset, metric, normal, missing, wrong in metrics_info:
        data['Metric Type'].append(metric_type)
        data['Dataset'].append(dataset)
        data['Metric'].append(metric)
        data['Normal (%)'].append(f'{normal:.2f}')
        data['Missing (%)'].append(f'{missing:.2f}')
        data['Wrong (%)'].append(f'{wrong:.2f}')
    
    # Create DataFrame and print
    table_df = pd.DataFrame(data)
    
    print(f"\n{'='*80}")
    print(f"{chart_title:^80}")
    print(f"{'='*80}\n")
...
def main():
    ...
    table_df = create_combined_precision_recall_chart(
        results_df,
